...
### Testing the Code​ <a href="#testing-the-code" id="testing-the-code"></a>
...
### Write the Script​ <a href="#write-the-script" id="write-the-script"></a>
...
{% hint style="info" %}
For a full list of arguments that you can pass to the script, see more information [here](broken-reference)
{% endhint %}
...
The Bacalhau command passes a prompt to the model and generates an image in the outputs directory. The main difference in the example below compared to all the other examples is the addition of the `--gpu X` flag, which tells Bacalhau to only schedule the job on nodes that have `X` GPUs free. You can [read more about GPU support](broken-reference) in the documentation.
